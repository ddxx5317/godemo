...
func StructQueryField() {
	user := new(User)
	row := utils.MysqlDb.QueryRow("select id, name, age from users where id=?",1)
...
func StructQueryAllField() {
	...
	// 通过切片存储
	users := make([]User, 0)
	rows, _:= utils.MysqlDb.Query("SELECT * FROM `users` limit ?",100)
	// 遍历
	var user User
	for  rows.Next(){
...
func StructInsert() {
	...
	ret,_ := utils.MysqlDb.Exec("insert INTO users(name,age) values(?,?)","小红",23)
...
func StructUpdate() {
	...
	ret,_ := utils.MysqlDb.Exec("UPDATE users set age=? where id=?","100",1)
...
func StructDel() {
	...
	ret,_ := utils.MysqlDb.Exec("delete from users where id=?",1)
...
func StructTx() {
	...
	//事务处理
	tx, _ := utils.MysqlDb.Begin()


	// 新增
	userAddPre, _ := utils.MysqlDb.Prepare("insert into users(name, age) values(?, ?)");
...
func RawQueryField() {
	...
	rows, _ := utils.MysqlDb.Query("select id,name from users")
	if rows == nil {
		return
	}
	id := 0
...
func RawQueryAllField() {
	...
	//查询数据，取所有字段
	rows2, _ := utils.MysqlDb.Query("select * from users")
